...
import { CourseUnitFilters } from "@/types/course-unit";

// Query keys
export const courseUnitKeys = {
  all: ["course-units"] as const,
  lists: () => [...courseUnitKeys.all, "list"] as const,
  list: (filters?: CourseUnitFilters) => [...courseUnitKeys.lists(), filters] as const,
...
// Hooks
export const useCourseUnits = (filters?: CourseUnitFilters) => {
  return useQuery({
    queryKey: courseUnitKeys.list(filters),
    queryFn: () => getCourseUnits(filters),
  });
};
