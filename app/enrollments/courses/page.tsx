...
import type { CourseEnrollmentFilters, CreateCourseEnrollmentInput } from "@/types/course-enrollment";
...
export default function EnrollmentCoursesPage() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ isOpen: boolean; courseEnrollmentId: string | null }>({
    isOpen: false,
            ...
            <div>
              ...
              {enrollment && (
                <p className="mt-1 text-sm text-zinc-500">
                  {enrollment.student?.full_name} ({enrollment.student?.student_number}) - {enrollment.academic_program?.name}
                </p>
              )}
            </div>
          </div>
          ...
          <>
            ...
            <div className="mt-6 flex items-center justify-between border-t border-zinc-200 bg-white px-6 py-4">
              <p className="text-sm text-zinc-500">
                Affichage de {data ? ((data.page - 1) * data.limit) + 1 : 0} sur {data?.total ?? 0} cours
              </p>
